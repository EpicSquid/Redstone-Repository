...
public class ItemAxeGelidEnderium extends ItemAxeFlux {
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if (!world.isRemote && hand == EnumHand.MAIN_HAND && this.isEmpowered(held) && (world.isRaining() || world.isThundering())) {
            ...
            world.spawnEntity((Entity)new EntityLightningBolt(world, player.posX, player.posY, player.posZ, true));
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (this.getEnergyStored(held) > 6400) {
            if (!this.isEmpowered(held)) {
                world.spawnEntity((Entity)new EntityLightningBolt(world, (double)x, (double)y, (double)z, false));
                if (!player.capabilities.isCreativeMode) {
    ...
    protected boolean harvestBlock(World world, BlockPos pos, EntityPlayer player) {
        ...
        IBlockState state = world.getBlockState(pos);
        Block block = state.getBlock();
        if (!this.toolClasses.contains((Object)state.getBlock().getHarvestTool(state)) && !this.canHarvestBlock(state, player.getHeldItemMainhand())) {
            return false;
        }
        ...
        if (!world.isRemote) {
            int xpToDrop = 0;
            if (playerMP != null && (xpToDrop = ForgeHooks.onBlockBreakEvent((World)world, (GameType)playerMP.interactionManager.getGameType(), (EntityPlayerMP)playerMP, (BlockPos)pos)) == -1) {
                return false;
            }
            ...
            playerMP.connection.sendPacket((Packet)new SPacketBlockChange(world, pos));
        } else {
            if (block.removedByPlayer(state, world, pos, player, !player.capabilities.isCreativeMode)) {
                block.onBlockDestroyedByPlayer(world, pos, state);
            }
            Minecraft.getMinecraft().getConnection().sendPacket((Packet)new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, pos, Minecraft.getMinecraft().objectMouseOver.sideHit));
        }
        return true;
    }
    ...
    public static class CutTreeTask {
        ...
        public void cutTree(TickEvent.WorldTickEvent event) {
            ...
            while (blocksIter > 0) {
                ...
                if (!this.visited.add(curPos) || !(state = this.world.getBlockState(curPos)).getBlock().isWood((IBlockAccess)this.world, curPos) || (refStrength = state.getPlayerRelativeBlockHardness(this.player, this.world, curPos)) == 0.0f) continue;
                for (EnumFacing face : cardinals = new EnumFacing[]{EnumFacing.NORTH, EnumFacing.EAST, EnumFacing.SOUTH, EnumFacing.WEST}) {
                    ...
                    if (this.visited.contains((Object)newPos)) continue;
